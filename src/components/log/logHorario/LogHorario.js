...
useEffect(() => {
    fetch("http://localhost:8080/api/log/logHorario")
      .then((resp) => resp.json())
      .then((retorno_convertido) => setLogHorario(retorno_convertido)); //lista de turmas
  }, []);
            ...
            <TableBody>
                ...
                {logHorario.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(
                    ({nomeUsuario, hora, data, logsEnum, nifUsuario}) => (
                        <StyledTableRow>
                             <StyledTableCell>O usuário {nomeUsuario} {logsEnum} um Horário</StyledTableCell>
