...
export default function LogHorario() {

    const classes = useStyles();

    const [logHorario, setLogHorario] = useState([])
    ...
    const getLog = async () => {
        let result = await fetch(`http://localhost:8080/api/log/logHorario`)
        result = await result.json();
        setLogHorario(result)
    ...
    const atualizaLista = async () => {
        const result = await fetch("http://localhost:8080/api/log/logHorario"); // await = espera uma promessa
        const resultado = await result.json();
        setLogHorario(resultado);
      };

    const buscaHorario = async (event) => {
        ...
        if (key) {
          ...
          let result = await fetch(
            "http://localhost:8080/api/log/buscarHorario/", {
                    ...
                    <TableContainer component={Paper}>
                        <Table sx={{ minWidth: 700 }} aria-label="customized table">
                            ...
                            <TableBody>
                                ...
                                {logHorario.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(
                                                    ...
                                                    <>

                                                        {
                                                            logsEnum === "CADASTROU"
                                                                ?
                                                                <>
                                                                    <StyledTableCell>{nomeUsuario} {logsEnumString} o seguinte horário: {informacaoCadastro}</StyledTableCell>
