...
export default function LogUsuario() {
    ...
    const atualizaLista = async () => {
        const result = await fetch("http://localhost:8080/api/log/logUsuario"); // await = espera uma promessa
        const resultado = await result.json();
        setLogUsuario(resultado);
      };
    ...
    const buscaUsuario = async (event) => {
        ...
        if (key) {
          ...
          let result = await fetch(
            "http://localhost:8080/api/log/buscarUsuario/", {
